...
import { ArrowLeft, Edit3, Save, X, Calendar, Bell, Tag, Activity } from 'lucide-react';
...
export default function Profile() {
  ...
  const [activeTab, setActiveTab] = useState<'events' | 'activity' | 'settings'>('events');
  ...
  useEffect(() => {
    ...
    if (userId) {
      api.get(`/users/${userId}`).then(r => {
        ...
        // If API fails, navigate to create profile
  ...
  const handleSave = async () => {
    if (!profile) return;
    try {
      const res = await api.put(`/users/profile/${profile.id}`, {
        name: form.name,
        role: form.role,
        contact: form.contact,
        hobbies: form.tags.split(',').map(t => t.trim()).filter(Boolean),
      });
      setProfile({ ...profile, ...res.data });
      setForm({ name: res.data.name || '', role: res.data.role || '', contact: res.data.contact || '', tags: (res.data.hobbies || []).join(', ') });
      setEditMode(false);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen px-4 py-4 sm:p-6 page-enter" style={{ background: 'linear-gradient(165deg, #e0f8f5 0%, #b8f0e9 25%, #a0eae3 50%, #88e3dd 75%, #6dd9d6 100%)' }}>
      <div className="max-w-md mx-auto space-y-4 sm:space-y-5">
        ...
        {/* Кнопка назад */}
        <button onClick={() => navigate(-1)} className="flex items-center justify-center w-11 h-11 bg-white/70 backdrop-blur-md rounded-xl text-teal-700 hover:bg-teal-50 transition-all shadow-md">
          <ArrowLeft size={18} />
        </button>
        ...
        <div className="profile-card">
          <div className="flex items-center gap-4 mb-5">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center text-white text-2xl font-bold shadow-lg">
              {profile.name?.charAt(0) || profile.firstName?.charAt(0) || '?'}
            </div>
            <div className="flex-1">
              <h2 className="text-xl font-bold text-teal-800">{profile.name || profile.firstName || 'Без имени'}</h2>
              <p className="text-sm text-teal-600">{profile.role || profile.profession || 'Профессия не указана'}</p>
              {profile.contact && <p className="text-sm text-teal-700 font-medium mt-1">{profile.contact}</p>}
              {profile.age && <p className="text-xs text-teal-500 mt-0.5">{profile.age} лет</p>}
            </div>
            {!editMode ? (
          ...
          {/* Tags display */}
          {(profile.hobbies?.length > 0 || profile.tags?.length > 0) && (
            <div className="flex flex-wrap gap-1.5 mb-4">
              {(profile.hobbies || profile.tags || []).map((tag: string, i: number) => (
                <span key={i} className="text-xs font-medium bg-teal-100 text-teal-700 px-2.5 py-1 rounded-full">{tag}</span>
              ))}
            </div>
          )}
        ...
        {/* Вкладки */}
        <div className="flex gap-1.5 p-1.5 bg-white/60 backdrop-blur-md rounded-xl shadow-md" style={{ boxShadow: '0 6px 20px rgba(0,188,212,0.12)' }}>
          {(['events', 'activity', 'settings'] as const).map(key => (
            <button key={key} onClick={() => setActiveTab(key)}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === key
        ...
        {activeTab === 'events' && (
          <div className="space-y-3 pb-8">
            {profile.events && profile.events.length > 0 ? (
              ...
              <div className="text-center py-8 text-teal-500 text-sm">
        ...
        {activeTab === 'activity' && (
          <div className="space-y-2 pb-8">
            {profile.activity && profile.activity.length > 0 ? (
              profile.activity.map((item: any, i: number) => (
